...
fn naca0012_coords() -> (Vec<f64>, Vec<f64>) {
    // 160-point NACA 0012 distribution (same as xfoil-python's test suite)
    let x = [
...
fn main() {
    ...
    println!("Analyzing NACA 0012 at Re = 1e6, M = 0");
    println!();
    println!("   alpha      CL        CD        CM      Cpmin   converged");
    println!("  ---------------------------------------------------------");

    let mut csv = String::from("alpha,CL,CD,CM,Cpmin,converged\n");
    for alpha in -20..=20 {
        let a = alpha as f64;
        let (cl, cd, cm, cp, conv) = xf.a(a);
        println!("  {:6.1}  {:9.4}  {:9.5}  {:9.4}  {:8.4}  {}", a, cl, cd, cm, cp, conv);
        csv.push_str(&format!("{:.1},{:.6},{:.6},{:.6},{:.6},{}\n", a, cl, cd, cm, cp, conv));
    }

    std::fs::write("naca0012-polar.csv", csv).expect("failed to write polar file");
    println!();
    println!("Polar written to naca0012-polar.csv");

    // Reference point check (matches xfoil-python test suite):
    xf.set_max_iter(100);
    let (cl, cd, cm, _cp, conv) = xf.a(10.0);
    println!();
    println!("alpha = 10 deg: CL = {:.4} (ref 1.0809), CD = {:.4} (ref 0.0150), CM = {:.4} (ref 0.0053), converged = {}", cl, cd, cm, conv);
}
