...
def iterate(foil_simulator, c, dv, a_prime, theta, omega, r, dr, u_0, B):
    ...
    dv_new = (
        -B
        * c
        * (C_D * (dv + u_0) + C_L * omega * r * (a_prime - 1))
        * sqrt(omega ** 2 * r ** 2 * (a_prime - 1) ** 2 + (dv + u_0) ** 2)
        / (4 * pi * (dr + 2 * r) * (dv + u_0))
    )
...
def min_func2(x, theta, omega, r, dr, u_0, B, foil_simulator):
    ...
    C_L, C_D, phi = precalc(foil_simulator, dv, a_prime, theta, omega, r, dr, u_0, B)
    return lsq(
        C_L, C_D, foil_simulator.foil.chord, dv, a_prime, theta, omega, r, dr, u_0, B
    )
...
def dv_from_thrust(T, R, u_0, rho=1.225):
    dv = -u_0 / 2 + sqrt(pi * R ** 2 * rho ** 2 * u_0 ** 2 + 2 * T * rho) / (
        2 * sqrt(pi) * R * rho
    )
    return dv
...
def error(dv, dv2, a_prime, a_prime2):
    return abs((dv - dv2) / (dv + dv2)) + abs(
        (a_prime - a_prime2) / (a_prime + a_prime2)
    )
...
def min_func(x, theta, omega, r, dr, u_0, B, foil_simulator):
    dv, a_prime = x
    try:
        dv2, a_prime2 = iterate(
            foil_simulator,
            foil_simulator.foil.chord,
            dv,
            a_prime,
            theta,
            omega,
...
def bem_iterate(foil_simulator, dv_goal, theta, rpm, r, dr, u_0, B):
    ...
    res = minimize(
        ...
        x0,
        jac=jac_func2,
        args=(theta, rpm2omega(rpm), r, dr, u_0, B, foil_simulator),
        method="SLSQP",
...
def design_for_dv(foil_simulator, dv_goal, rpm, r, dr, u_0, B):
    C_L, C_D, phi = precalc(
        foil_simulator, dv_goal, 0, 0, (rpm / 60) * 2 * pi, r, dr, u_0, B
    )
    ...
    res = minimize(
        min_dv,
        x0,
        args=(dv_goal, rpm, r, dr, u_0, B, foil_simulator),
        tol=1e-10,  # method='COBYLA', constraints=constraints, options={'disp': True, 'maxiter': 1000})
        method="SLSQP",
        constraints=constraints,
...
def min_all(x, goal, rpm, r, dr, u_0, B, foil_simulator):
    ...
    try:
        omega = rpm2omega(rpm)
        dv2, a_prime2 = iterate(
            foil_simulator, chord, dv, a_prime, theta, omega, r, dr, u_0, B
        )
        err = error(dv, dv2, a_prime, a_prime2)
...
def optimize_all(foil_simulator, dv_goal, rpm, r, dr, u_0, B, maxchord):
    ...
    logger.info(
        "dv: {}, goal: {} a_prime={}, chord={}".format(
            res.x[1], dv_goal, res.x[2], res.x[3]
        )
    )
    return res.x, res.fun
...
def prop_design(
    R0=2.0 / 100, R=10.0 / 100, tip_chord=0.01, dr=0.005, u_0=0.0, rpm=10000.0
):
    ...
    for r in arange(R0, R, dr):
        ...
        print(
            (
                "r={}, theta={}, dv={}, a_prime={} \t:err={} ".format(
                    r * 100, degrees(theta), dv, a_prime, fun
                )
            )
        )
