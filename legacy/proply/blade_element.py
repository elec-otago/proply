...
class BladeElement:
    ...
    def dM(self):
        return optimize.dM(
            self.dv, self.a_prime, self.r, self.dr, self.omega, self.u_0, rho=1.225
        )

    def bem(self, n_blades):
        logger.info("bem {}".format(self))
        dv, a_prime, err = optimize.bem_iterate(
            foil_simulator=self.fs,
            dv_goal=self.dv,
            theta=self._twist,
            rpm=self.rpm,
            B=n_blades,
    ...
    def get_foil_points(self, n, scimitar_offset):
        ...
        theta_l = (
            2.0 * np.pi * yl / c + scimitar_angle
        )  # angular coordinate along circumference (fraction)
        xl = r * np.cos(theta_l)
        yl = r * np.sin(theta_l)

        theta_u = (
            2.0 * np.pi * yu / c + scimitar_angle
        )  # angular coordinate along circumference (fraction)
        xu = r * np.cos(theta_u)
        yu = r * np.sin(theta_u)
    ...
    def __repr__(self):
        dt = self.dT()
        dm = self.dM()
        return "BladeElement(r={:5.3f}, twist={:5.2f}, foil[{}], dv={:4.1f}, eff={:4.1f})".format(
            self.r, np.degrees(self._twist), self.foil, self.dv, dt / dm
        )
...
if __name__ == "__main__":
    ...
    f = NACA4(
        chord=0.1, thickness=0.15, m=0.06, p=0.4, angle_of_attack=8.0 * np.pi / 180.0
    )
